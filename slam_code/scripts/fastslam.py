    ...
    def correction_step(self, sensor_measurements):
        ...
        for measurement in sensor_measurements:
            ...
            if not landmark.observed:
                phi = normalize_angle(robot_pose[2] + measurement.z_bearing)  # robot pose + landmark bearing
                landmark.mu = np.vstack([
                    robot_pose[0] + measurement.z_range * np.cos(phi),  # x + r * cos(phi)
                    robot_pose[1] + measurement.z_range * np.sin(phi)  # y + r * sin(phi)
                ])
                ...
                z_diff = np.vstack([measurement.z_range - expected_z[0], normalize_angle(measurement.z_bearing - expected_z[1])])

                landmark.mu = landmark.mu + K @ z_diff
                landmark.sigma = (np.identity(2) - K @ H) @ landmark.sigma

                w = np.power(np.linalg.det(2 * np.pi * Q), -0.5) * np.exp(-0.5 * z_diff.T @ np.linalg.inv(Q) @ z_diff)
